...
public class ExcelParserTest<T> {
    ...
    public final void testParseDocument() {
        try {
            List<T> list = getInstance().parseDocument(getSheetToTest());
            assertNotNull(list);
            ...
            assertEquals("The first element must be the first element expected", getFirstElementExpected(), list.get(0));
            assertEquals("The last element must be the last element expected", getLastElementExpected(), list.get(list.size() - 1));
    ...
    protected Sheet getSheetToTest() {
        return new SheetBuilder(new XSSFWorkbook(), new String[][]{{"Header1", "Header2"}, {"Key1", "Data1"}, {"Key2", "Data2"}}).build();
    }
